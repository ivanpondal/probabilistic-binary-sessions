...
module Bare = struct
  ...
  let ( @> ) scope ep = snd ((fun ep -> ((), scope ep)) @= ep)
end
...
module Monadic = struct
  ...
  let send x ep =
    let ep = Bare.send x ep in
    ((), ep)

  let select_true ep =
    let ep = Bare.select_true ep in
    ((), ep)

  let select_false ep =
    let ep = Bare.select_false ep in
    ((), ep)

  let branch m1 m2 ep =
    match Bare.branch ep with `True ep -> m1 ep | `False ep -> m2 ep
end
