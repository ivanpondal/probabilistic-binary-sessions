...
Si retomamos nuestro ejemplo anterior, podemos observar que el tipado de
\OI{epY} en cada rama no era compatible bajo la definición del
combinador. Veamos qué sucede al utiliazar figuras compatibles:

\TwoSessionsInvalidPickBothBranchesWithSelect

Ahora \OI{epY} tiene tipo $\BinaryPChoice[0]{\Out\tbool\Done}{\SessionTypeS}$
en la rama $\Tag[False]$ y $\BinaryPChoice[1]{\SessionTypeT}{\Out\tint\Done}$
cuando se escoge $\Tag[True]$. Estos tipos son compatibles para la combinación,
aún así el programa no tipa ya que el motor de inferencia no dispone de la
