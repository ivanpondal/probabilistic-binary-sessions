...
X.\Out\tint\BinaryPBranch[\frac{1}{4}]{\Done}{\In\tint\BinaryPChoice[\frac{2}{3}]{\Idle}{X}}$.
	...
	\begin{tikzpicture}[scale=1.7]
		...
		\node[state, accepting] (s3) at (0, 1) {$s_3$};
		\node[state] (s4) at (2, 1) {$s_4$};
		\node[state] (s5) at (1, 0) {$s_5$};
		\draw [->] (s0) to node[right, p]{$1$} (s1);
		\draw [->] (s1) to node[right, p]{$1$} (s2);
		\draw [->] (s2) to node[above, p]{$\frac{1}{4}$} (s3);
		\draw [->] (s2) to node[above, p]{$\frac{3}{4}$} (s4);
		\draw [->] (s4) to node[above, p]{$\frac{2}{3}$} (s5);
		\draw [->] (s4) to [bend right] node[right, p]{$\frac{1}{3}$} (s0.east);
    \end{tikzpicture}
